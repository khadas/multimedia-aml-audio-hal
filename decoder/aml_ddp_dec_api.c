...
static const short channel_num[MAXCHANCFGS] = { 2, 1, 2, 3, 3, 4, 4, 5 };
enum {
    MODE11 = 0,
    MODE_RSVD = 0,
    MODE10,
...
int dcv_decode_ad_process(aml_dec_t * aml_dec, struct audio_buffer *abuffer)
{
...
int dcv_decoder_process_patch(aml_dec_t * aml_dec, struct audio_buffer *abuffer)
{
    ...
    ddp_dec->dcv_pcm_written += ddp_dec->outlen_pcm;
    ddp_dec->dcv_decoded_samples = (ddp_dec->dcv_pcm_written * 8 ) / (2 * bit_width);
