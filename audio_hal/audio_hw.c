                                ...
                                const char *address)
{
    ...
    if (flags & AUDIO_OUTPUT_FLAG_PRIMARY) {
        ...
        if (flags & AUDIO_OUTPUT_FLAG_HW_AV_SYNC) {
            ALOGI("Set out->hw_sync_mode PRIMARY true");
...
//#ifdef ENABLE_MMAP
    ...
    if (out->hw_sync_mode) {
        ...
        if (adev->active_inport == INPORT_HDMIIN || adev->active_inport == INPORT_LINEIN ||
            (adev->active_inport == INPORT_TUNER && adev->patch_src == SRC_ATV)) {
...
//#ifdef ENABLE_MMAP
    ...
    if (out->write_func == MIXER_MAIN_BUFFER_WRITE) {
        ...
        if (out->hw_sync_mode && continuous_mode(adev)) {
...
void config_output(struct audio_stream_out *stream, bool reset_decoder)
{
    ...
    if (eDolbyMS12Lib == adev->dolby_lib_type) {
        ...
        if (is_need_clean_up_ms12(stream, reset_decoder_stored)) {
            ...
            if (continuous_mode(adev) && !dolby_stream_active(adev)) {
                /*dummy we support it is  DD+*/
                aformat = AUDIO_FORMAT_E_AC3;
                main1_dummy = true;
            }
            if (continuous_mode(adev) && (hwsync_lpcm_active(adev) || !is_dolby_ms12_support_compression_format(aformat))) {
                ott_input = true;
            }
            if (continuous_mode(adev)) {
            ...
            if (continuous_mode(adev) && main1_dummy && !adev->is_netflix && !is_multi_channel_pcm(stream)) {
                set_ms12_acmod2ch_lock(&adev->ms12, true);
            }
...
ssize_t mixer_main_buffer_write(struct audio_stream_out *stream, struct audio_buffer *abuffer)
{
    ...
    if (adev->audio_patch && aml_out->tv_src_stream && IS_DIGITAL_IN_HW(patch->input_src)) {
        ...
        if (cur_aformat != patch->aformat && cur_aformat != AUDIO_FORMAT_INVALID) {
            ...
            if (cur_aformat == AUDIO_FORMAT_AC3 || cur_aformat == AUDIO_FORMAT_E_AC3) {
                int audio_samplerate = audio_parse_get_audio_samplerate(patch->audio_parse_para);
                if (audio_samplerate == 96000 || audio_samplerate == 192000) {
                    audio_samplerate = 48000;
                } else if (audio_samplerate == 88200 || audio_samplerate == 176400) {
            ...
            if ((adev->hdmi_format == BYPASS) && (ms12->ms12_bypass_handle)) {
            ...
            for (int i = 0; i < BITSTREAM_OUTPUT_CNT; i++) {
...
hwsync_rewrite:
    ...
    */
    if (aml_out->hw_sync_mode && return_bytes > 0) {
        /*
