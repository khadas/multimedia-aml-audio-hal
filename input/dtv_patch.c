...
#define PTSERVINSID 12 // make an agreement witch mediasync
...
static int32_t PtsServ_ioctl(int32_t PServerDevId,
                             int32_t PServerCmd,
                             uint64_t param) {
    int32_t ret = -1;
    if (PServerDevId <= 0) {
...
static int32_t PtsServ_close(int PServerDev)
{
    int32_t ret = -1;
    if (PServerDev < 0) {
        ...
        case AUDIO_DTV_PATCH_CMD_CONTROL:
            ...
            if (val == AUDIO_DTV_PATCH_CMD_OPEN) {
                if (adev->is_multi_demux) {
                    ...
                    {
                        pthread_mutex_lock(&dtvsync->lock);
                        dtvsync->mediasync_handle = mediasync_wrap_create();
                        if (dtvsync->mediasync_handle == NULL)
                            ...
                            mediasync_wrap_setParameter(dtvsync->mediasync_handle, MEDIASYNC_KEY_ISOMXTUNNELMODE, &audio_sync_mode);
                            mediasync_wrap_bindInstance(dtvsync->mediasync_handle, dtvsync->mediasync_id, MEDIA_AUDIO);
                            AM_LOGI("normal output version CMD open audio bind syncId:%d", dtvsync->mediasync_id);
                            mediasync_wrap_setParameter(dtvsync->mediasync_handle, MEDIASYNC_KEY_HASAUDIO, &has_audio);
                        }
                        pthread_mutex_unlock(&dtvsync->lock);
                    }
                    AM_LOGI("create mediasync:%p", dtvsync->mediasync_handle);
                } else {
                ...
                AM_LOGI("AUDIO_DTV_PATCH_CMD_CLOSE demux_hanle %p,%d,%p, ad_dmx_init: %d\n",demux_handle,adev->is_multi_demux, dtvsync->mediasync_handle, demux_info->ad_dmx_init);
                ...
                if (adev->is_multi_demux) {
                    if (demux_handle) {
                        ...
                        /*pthread_mutex_lock(&dtvsync->lock);
                        if ((dtvsync->mediasync_handle != NULL)) {
                            AM_LOGI("close mediasync_handle:%p, mediasync:%p", dtvsync->mediasync_handle, dtvsync->mediasync);
                            mediasync_wrap_destroy(dtvsync->mediasync_handle);
                            dtvsync->mediasync_handle = NULL;
...
{
    int ret;
    int64_t outRealMediaMs;
    int latencyms = -1;
    int64_t last_queue_es_apts = 0;
    struct aml_audio_patch *patch = aml_dev->audio_patch;

    do {
        if ((NULL == patch) || (NULL == patch->dtvsync) || (0 == aml_dev->is_multi_demux)) {
            AM_LOGW("dtv patch:%p, aml_dev->is_multi_demux:%d", patch, aml_dev->is_multi_demux);
            break;
        }

        ret = Get_Audio_LastES_Apts(patch->demux_handle, &last_queue_es_apts);
        if ((0 != ret) || (0 > last_queue_es_apts)) {
            AM_LOGW("Get_Audio_LastES_Apts, ret:%d", ret);
            break;
        }

        ret = mediasync_wrap_GetMediaTimeByType(patch->dtvsync->mediasync_handle, MEDIA_STC_TIME, MEDIASYNC_UNIT_MS, &outRealMediaMs);
        if ((0 != ret) || (0 > outRealMediaMs)) {
            AM_LOGW("mediasync_wrap_GetMediaTimeByType, ret:%d", ret);
            break;
        }
        latencyms = patch->last_chenkin_apts / 90 - outRealMediaMs;
    }while (0);
...
#define T5DDEFAULTMAINSIZE 1024
void *audio_dtv_patch_input_threadloop(void *data)
{
    ...
    AM_LOGI("patch->input_thread_exit %d, read_bytes %d!!!", patch->input_thread_exit, read_bytes);
    ...
    while (!patch->input_thread_exit) {
        ...
        if (!aml_dev->is_multi_demux) {
            ...
            if (dtv_ad_package == NULL && check_ad_enable(patch)) {
                 ...
                 memset(dtv_ad_package, 0, sizeof(struct package));
                 AM_LOGI_IF(aml_dev->debug_flag, "input, ad package %p alloc", dtv_ad_package);
            }

            int nNextReadSize = read_bytes; //full buffer size//GetInputSizeandpts(dtv_audio_instances,&(dtv_package->pts),patch);
            rlen = 0;
            ...
            dtv_package->data = aml_audio_malloc(nNextReadSize);
            if (!dtv_package->data) {
                AM_LOGE("inbuf malloc failed");
                pthread_mutex_unlock(&patch->mutex);
                goto exit;
            }
            memset(dtv_package->data, 0, nNextReadSize);
            while (nNextReadSize > 0 && !patch->input_thread_exit) {
                char buf[4096] = {0};
                sysfs_get_sysfs_str("/sys/class/amstream/bufs", buf, sizeof(buf));
                nRet = uio_read_buffer((unsigned char *)(dtv_package->data + rlen), nNextReadSize, patch->input_thread_exit);
                AM_LOGI_IF(aml_dev->debug_flag, "uio_read_buffer nRet:%d rlen %d nNextReadSize %d(%d), amstream bufs=%s", nRet, rlen, nNextReadSize, patch->in_read_frame_size, buf);
                if (nRet <= 0) {
                    trycount++;
                    usleep(3000);
                    //continue;
            ...
            if (check_ad_enable(patch)) {
                ...
                if (nRet == AM_AUDIO_Dmx_SUCCESS && dtv_ad_package && mAdEsData) {
            ...
            while (!patch->input_thread_exit && dtv_package) {
                ...
                if (ret == 0) {
                    AM_LOGI_IF(aml_dev->debug_flag, "cond_signal main package %p, sz %d, pts %"PRIu64", num %d", dtv_package, dtv_package->size, dtv_package->pts, list->pack_num);
            ...
            while (!patch->input_thread_exit && dtv_ad_package) {
                ...
                if (ret == 0) {
                    AM_LOGI_IF(aml_dev->debug_flag, "cond_signal ad package %p, sz %d, pts %"PRIu64", num %d", dtv_ad_package, dtv_ad_package->size, dtv_ad_package->pts, ad_list->pack_num);
                    pthread_cond_signal(&patch->ad_cond);
                    pthread_mutex_unlock(&patch->assoc_mutex);
                    dtv_ad_package = NULL;
                    break;
                } else {
                    AM_LOGI("ad list->pack_num %d full !!!", ad_list->pack_num);
                    pthread_mutex_unlock(&patch->assoc_mutex);
                    usleep(150000);
                    continue;
                }
                ...
                if (demux_handle == NULL) {
                    ...
                    if (dtv_package == NULL) {
                     ...
                     /* mediasyn check dmx package */
                     {
                        ...
                        mediasync_wrap_queueAudioFrame(Dtvsync->mediasync_handle, &audio_queue_info);
                        AM_LOGI_IF(aml_dev->debug_flag, "queue pts:0x%"PRIx64", size:%d, isneedupdate %d, ret %d\n",
                                 dtv_package->pts, dtv_package->size,audio_queue_info.isneedupdate, nRet);
                        if (path_index != dtv_audio_instances->demux_index_working) {
...
void *audio_dtv_patch_output_threadloop(void *data)
{
    ...
    */
    ...
    if (aml_out->avsync_ctx)
    {
        ...
        aml_out->avsync_ctx->mediasync_ctx->handle       = patch->dtvsync->mediasync_handle;
    ...
    uint32_t ptsserver_list_size = 0;
    bool ptsserver_has_data = false;
        ...
        if (!aml_dev->is_multi_demux)
        {
            ...
            if (patch->PServerDev != -1) {
                ...
                if (checkout_pts.pts_90k != -1) {
                    patch->last_valid_pts = checkout_pts.pts_90k;
                    patch->cur_package->pts = checkout_pts.pts_90k;
                }
                if (checkout_pts.pts_90k == -1) {
                    if (aml_out->aml_dec && aml_out->aml_dec->out_frame_pts) {
                        patch->cur_package->pts = aml_out->aml_dec->out_frame_pts;
                    } else {
                        patch->cur_package->pts = patch->last_valid_pts;
                    }
                }
            }
        ...
        case AUDIO_DTV_PATCH_DECODER_STATE_START:
            ...
            if (cmd == AUDIO_DTV_PATCH_CMD_START) {
                ...
                if (dtvsync->mediasync_handle != NULL) {
                    ...
                    mediasync_wrap_setParameter(dtvsync->mediasync_handle, MEDIASYNC_KEY_AUDIOFORMAT, &audio_format);
                }
                ...
                patch->dtv_pcm_readed = patch->dtv_pcm_written = 0;
        ...
        case AUDIO_DTV_PATCH_DECODER_STATE_RUNNING:
            ...
            if (cmd == AUDIO_DTV_PATCH_CMD_PAUSE) {
                ...
                if (aml_dev->is_multi_demux) {
                    ...
                    patch->demux_handle = dtv_audio_instances->demux_handle[path_id];
                    aml_dtvsync_t *dtvsync = &dtv_audio_instances->dtvsync[path_id];
                    if (dtvsync->mediasync_handle) {
                        mediasync_wrap_setPause(dtvsync->mediasync_handle, true);
                    }
                }  else {
        ...
        case AUDIO_DTV_PATCH_DECODER_STATE_PAUSE:
            ...
            if (cmd == AUDIO_DTV_PATCH_CMD_RESUME) {
                if (aml_dev->is_multi_demux) {
                    aml_dtvsync_t *dtvsync = &dtv_audio_instances->dtvsync[path_id];
                    if (dtvsync->mediasync_handle) {
                        mediasync_wrap_setPause(dtvsync->mediasync_handle, false);
                    }
                } else {
                    aml_dtvsync_t *dtvsync = &dtv_audio_instances->dtvsync[0];
                    if (dtvsync->mediasync_handle) {
                       ...
                       audio_devices_t output __unused)
{
    ...
    if (!aml_dev->is_multi_demux) {
        ...
        if (patch->PServerDev != -1) {
            ...
            PtsServ_ioctl(patch->PServerDev, PTSSERVER_IOC_INSTANCE_SET_ID, (unsigned long)&PServerInsId);
            PtsServ_ioctl(patch->PServerDev, PTSSERVER_IOC_INSTANCE_STATIC_BINDER, (unsigned long)&mAllocPara);
            AM_LOGI("PServerDev %d\n", patch->PServerDev);
         }
    }
