...
{
    struct aml_ac3_parser *parser_handle = NULL;

    parser_handle = (struct aml_ac3_parser *)aml_audio_calloc(1, sizeof(struct aml_ac3_parser));
    if (parser_handle == NULL) {
        ALOGE("%s handle error", __func__);
        goto error;
    }

    parser_handle->buf_size  = DOLBY_DDPP_MAXSIZE;
    parser_handle->buf  = aml_audio_calloc(1, DOLBY_DDPP_MAXSIZE);
...
{
    struct aml_ac3_parser *ac3_parser_handle = (struct aml_ac3_parser *)parser_handle;

    if (ac3_parser_handle) {
        if (ac3_parser_handle->buf) {
            aml_audio_free(ac3_parser_handle->buf);
        }
        aml_audio_free(ac3_parser_handle);
    }
...
int aml_ac3_parser_reset(void *parser_handle)
{
    ...
    if (ac3_parser_handle) {
        ac3_parser_handle->status = PARSER_SYNCING;
        ac3_parser_handle->buf_remain = 0;
    }
    ALOGE("%s exit", __func__);
    return 0;
}
...
int aml_ac3_parser_process(void *parser_handle, const void *in_buffer, int32_t numBytes, int32_t *used_size, const void **output_buf, int32_t *out_size, struct ac3_parser_info * ac3_info)
{
    struct aml_ac3_parser *ac3_parser_handle = (struct aml_ac3_parser *)parser_handle;
    size_t remain = 0;
    ...
    parser_buf = ac3_parser_handle->buf;
    buf_left     = numBytes;

    ALOGV("%s input buf size=%d status=%d", __func__, numBytes, ac3_parser_handle->status);

    /*we need at least 12 bytes*/
    if (ac3_parser_handle->buf_remain < DOLBY_DDP_HEADER_SIZE) {
        need_size = DOLBY_DDP_HEADER_SIZE - ac3_parser_handle->buf_remain;
        /*input data is not enough, just copy to internal buf*/
        if (buf_left < need_size) {
            memcpy(parser_buf + ac3_parser_handle->buf_remain, buffer + buf_offset, buf_left);
            ac3_parser_handle->buf_remain += buf_left;
            goto error;
        }
        /*make sure the remain buf has 12 bytes*/
        memcpy(parser_buf + ac3_parser_handle->buf_remain, buffer + buf_offset, need_size);
        ac3_parser_handle->buf_remain += need_size;
        buf_offset += need_size;
        buf_left   = numBytes - buf_offset;

    }

    if (ac3_parser_handle->status == PARSER_SYNCING) {
        sync_word_offset = -1;
        while (sync_word_offset < 0) {
            ...
            sync_word_offset = seek_dolby_sync_word((char*)parser_buf, ac3_parser_handle->buf_remain);
            /*if we don't find the header in period bytes, move the last 1 bytes to header*/
            if (sync_word_offset < 0) {
                memmove(parser_buf, parser_buf + ac3_parser_handle->buf_remain - 1, 1);
                ac3_parser_handle->buf_remain = 1;
                need_size = DOLBY_DDP_HEADER_SIZE - ac3_parser_handle->buf_remain;
                /*input data is not enough, just copy to internal buf*/
                if (buf_left < need_size) {
                    memcpy(parser_buf + ac3_parser_handle->buf_remain, buffer + buf_offset, buf_left);
        ...
        ac3_parser_handle->status = PARSER_SYNCED;

        data_valid = ac3_parser_handle->buf_remain - sync_word_offset;
        /*move the header to the beginning of buf*/
        if (sync_word_offset != 0) {
            memmove(parser_buf, parser_buf + sync_word_offset, data_valid);
        }
        ac3_parser_handle->buf_remain = data_valid;
    ...
    sync_word_offset = seek_dolby_sync_word((char*)parser_buf, ac3_parser_handle->buf_remain);
    if (sync_word_offset != 0) {
        ALOGE("we can't get here remain=%d,resync dolby header", ac3_parser_handle->buf_remain);
        ac3_parser_handle->buf_remain = 0;
        ac3_parser_handle->status = PARSER_SYNCING;
        goto error;
    }
     ...
     */
    ret = parse_dolby_frame_header(parser_buf, ac3_parser_handle->buf_remain,  &frame_offset, &ac3_info->frame_size,
    ...
    if ((ac3_parser_handle->buf_remain + buf_left) >= frame_size) {
        need_size = frame_size - (ac3_parser_handle->buf_remain);
        if (need_size >= 0) {
            new_buf_size = ac3_parser_handle->buf_remain + need_size;
            if (new_buf_size > ac3_parser_handle->buf_size) {
                ac3_parser_handle->buf = aml_audio_realloc(ac3_parser_handle->buf, new_buf_size);
                if (ac3_parser_handle->buf == NULL) {
                    ALOGE("%s realloc buf failed =%d", __func__, new_buf_size);
                    ac3_parser_handle->buf_remain = 0;
                    ac3_parser_handle->status = PARSER_SYNCING;
                    goto error;
                }
                ac3_parser_handle->buf_size = new_buf_size;
                parser_buf = ac3_parser_handle->buf;
        ...
        new_buf_size = ac3_parser_handle->buf_remain + buf_left;
        if (new_buf_size > ac3_parser_handle->buf_size) {
            ac3_parser_handle->buf = aml_audio_realloc(ac3_parser_handle->buf, new_buf_size);
            if (ac3_parser_handle->buf == NULL) {
                ALOGE("%s realloc buf failed =%d", __func__, new_buf_size);
        ...
        memcpy(parser_buf + ac3_parser_handle->buf_remain, buffer + buf_offset, buf_left);
        ac3_parser_handle->buf_remain += buf_left;
        ac3_parser_handle->status = PARSER_LACK_DATA;
        goto error;
    }
    if (ac3_parser_handle->framesize != frame_size) {
        ac3_parser_handle->framesize = frame_size;
