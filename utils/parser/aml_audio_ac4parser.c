...
{
    struct aml_ac4_parser *ac4_parser_handle = NULL;

    ac4_parser_handle = (struct aml_ac4_parser *)calloc(1, sizeof(struct aml_ac4_parser));
    if (ac4_parser_handle == NULL) {
        ALOGE("%s handle error", __func__);
        goto error;
    }

    ac4_parser_handle->buf_size   = DOLBY_AC4_MAX_FRAMESIZE;
    ac4_parser_handle->buf        = calloc(1, DOLBY_AC4_MAX_FRAMESIZE);
    if (ac4_parser_handle->buf == NULL) {
        ALOGE("%s data buffer error", __func__);
        free(ac4_parser_handle);
        ac4_parser_handle = NULL;
        goto error;
    }
    ac4_parser_handle->status     = PARSER_SYNCING;
    ac4_parser_handle->buf_remain = 0;
    *pparser_handle = ac4_parser_handle;
    ALOGI("%s exit =%p", __func__, ac4_parser_handle);
    return 0;
error:
    *pparser_handle = NULL;
...
int aml_ac4_parser_close(void *parser_handle)
{
    ...
    if (ac4_parser_handle) {
        if (ac4_parser_handle->buf) {
            free(ac4_parser_handle->buf);
        }
        free(ac4_parser_handle);
    }
...
int aml_ac4_parser_process(void *parser_handle, const void *in_buffer, int32_t numBytes, int32_t *used_size, void **output_buf, int32_t *out_size, struct ac4_parser_info * ac4_info)
{
    ...
    if (ac4_parser_handle == NULL) {
        ALOGE("error ac4_parser_handle is NULL");
        goto error;
    }
    ...
    parser_buf = ac4_parser_handle->buf;
    buf_left   = numBytes;

    ALOGV("%s input buf size=%d status=%d", __func__, numBytes, ac4_parser_handle->status);

    /*we need at least DOLBY_AC4_HEADER_SIZE bytes*/
    if (ac4_parser_handle->buf_remain < DOLBY_AC4_HEADER_SIZE) {
        need_size = DOLBY_AC4_HEADER_SIZE - ac4_parser_handle->buf_remain;
        /*input data is not enough, just copy to internal buf*/
        if (buf_left < need_size) {
            memcpy(parser_buf + ac4_parser_handle->buf_remain, buffer + buf_offset, buf_left);
            ac4_parser_handle->buf_remain += buf_left;
            goto error;
        }
        /*make sure the remain buf has DOLBY_AC4_HEADER_SIZE bytes*/
        memcpy(parser_buf + ac4_parser_handle->buf_remain, buffer + buf_offset, need_size);
        ac4_parser_handle->buf_remain += need_size;
        buf_offset += need_size;
        buf_left   = numBytes - buf_offset;

    }

    if (ac4_parser_handle->status == PARSER_SYNCING) {
        sync_word_offset = -1;
        while (sync_word_offset < 0) {
            ...
            sync_word_offset = seek_ac4_sync_word((char*)parser_buf, ac4_parser_handle->buf_remain);
            /*if we don't find the header in period bytes, move the last 1 bytes to header*/
            if (sync_word_offset < 0) {
                memmove(parser_buf, parser_buf + ac4_parser_handle->buf_remain - 1, 1);
                ac4_parser_handle->buf_remain = 1;
                need_size = DOLBY_AC4_HEADER_SIZE - ac4_parser_handle->buf_remain;
                /*input data is not enough, just copy to internal buf*/
                if (buf_left < need_size) {
                    memcpy(parser_buf + ac4_parser_handle->buf_remain, buffer + buf_offset, buf_left);
        ...
        ac4_parser_handle->status = PARSER_SYNCED;

        data_valid = ac4_parser_handle->buf_remain - sync_word_offset;
        /*move the header to the beginning of buf*/
        if (sync_word_offset != 0) {
            memmove(parser_buf, parser_buf + sync_word_offset, data_valid);
        }
        ac4_parser_handle->buf_remain = data_valid;
    ...
    sync_word_offset = seek_ac4_sync_word((char*)parser_buf, ac4_parser_handle->buf_remain);
    if (sync_word_offset != 0) {
        ALOGE("we can't get here remain=%d,resync dolby header", ac4_parser_handle->buf_remain);
        ac4_parser_handle->buf_remain = 0;
        ac4_parser_handle->status = PARSER_SYNCING;
        goto error;
    }
     ...
     */
    ret = parse_ac4_frame_header(&ac4_parser_handle->bit_parser, parser_buf, ac4_parser_handle->buf_remain, ac4_info);
    ...
    if ((ac4_parser_handle->buf_remain + buf_left) >= frame_size) {
        need_size = frame_size - (ac4_parser_handle->buf_remain);
        if (need_size >= 0) {
            new_buf_size = ac4_parser_handle->buf_remain + need_size;
            if (new_buf_size > ac4_parser_handle->buf_size) {
                void * new_buf = realloc(ac4_parser_handle->buf, new_buf_size);
                if (new_buf == NULL) {
                ...
                ac4_parser_handle->buf      = new_buf;
                ac4_parser_handle->buf_size = new_buf_size;
                parser_buf = ac4_parser_handle->buf;
        ...
        new_buf_size = ac4_parser_handle->buf_remain + buf_left;
        if (new_buf_size > ac4_parser_handle->buf_size) {
            void * new_buf = realloc(ac4_parser_handle->buf, new_buf_size);
            if (new_buf == NULL) {
                ALOGE("%s realloc buf failed =%d", __func__, new_buf_size);
        ...
        memcpy(parser_buf + ac4_parser_handle->buf_remain, buffer + buf_offset, buf_left);
        ac4_parser_handle->buf_remain += buf_left;
        ac4_parser_handle->status = PARSER_LACK_DATA;
        goto error;
    }
